...
function buildCard(c){
 ...
 if(c.image){img.src=c.image;img.alt=c.name;img.hidden=false;ph.hidden=true;img.onerror=()=>{img.hidden=true;ph.hidden=false};img.onclick=()=>openCardImage(c);}else{img.hidden=true;ph.hidden=false}
...
function renderLatest(){const c=$("#latestCards");if(!c)return;const selected=cards.filter(x=>owned[x.id]).slice(-5).reverse();const fallback=cards.filter(x=>["SLR","GDR","SCR","SPR","SR"].includes(x.rarity)).slice(0,5);c.innerHTML="";(selected.length?selected:fallback).forEach(card=>{const b=document.createElement("button");b.className="latest-card";b.innerHTML=`<img src="${card.image}" alt="${card.name}"><span>${card.number}</span><b class="rarity-${card.rarity}">${card.rarity}</b>`;b.onclick=()=>openCardImage(card);c.appendChild(b);});}
